...
# 	get_lobby_members()
# 	# Proactively accept sessions with all members when lobby data changes
# 	_ensure_p2p_sessions_with_members()
...
# 	var payload := {"voice_data": voice_data, "steam_id": steam_id, "username": steam_username}
# 	var host_id: int = 0
# 	if lobby_id != 0:
# 		host_id = Steam.getLobbyOwner(lobby_id)
# 	var send_type: int = Steam.P2P_SEND_RELIABLE
# 	# If I'm the host, broadcast directly to all others on voice channel
# 	if host_id != 0 and host_id == steam_id:
# 		send_p2p_packet(1, payload)
# 	else:
# 		# Non-host: send only to host for reliable relay, avoiding client-to-client duplicates
# 		if host_id != 0 and host_id != steam_id:
# 			var this_data: PackedByteArray
# 			this_data.append_array(var_to_bytes(payload))
# 			Steam.sendP2PPacket(host_id, this_data, send_type, 1)
# 		else:
# 			# Fallback: if host unknown, broadcast to all (legacy behavior)
# 			send_p2p_packet(1, payload)
...
# 			# Prefer the original sender embedded in payload; fallback to packet sender
# 			var original_sender_id: int = readable_data.get("steam_id", packet_sender)
# 			var players_in_scene: Array = get_tree().get_nodes_in_group("players")
# 			for player in players_in_scene:
# 				if player.steam_id == original_sender_id:
# 					# Access the VoiceChat child node to process the voice data
# 					var voice_chat = player.get_node_or_null("VoiceChat")
# 					if voice_chat and voice_chat.has_method("process_voice_data"):
# 						voice_chat.process_voice_data(readable_data, "network", readable_data["username"])
# 					else:
# 						print("WARNING: VoiceChat node not found or doesn't have process_voice_data method for player: ", player.steam_id)
# 					break
# 				else:
# 					# TODO: ERROR CHECKING
# 					pass

# 			# If we're the server/host, relay this voice packet to other clients (mesh via host)
# 			if multiplayer.is_server():
# 				_relay_voice_payload_from(original_sender_id, readable_data)
# #endregion
...
# 		# Allow Valve relay fallback on all peers to improve connectivity
...
# # Accept P2P sessions proactively with all lobby members so clients can receive each other
# func _ensure_p2p_sessions_with_members() -> void:
# 	for member in lobby_members:
# 		var member_steam_id: int = member.get("steam_id", 0)
# 		if member_steam_id != 0 and member_steam_id != steam_id:
# 			Steam.acceptP2PSessionWithUser(member_steam_id)

# # Relay voice packets from the original sender to all other clients except self and the sender
# func _relay_voice_payload_from(original_sender_id: int, payload: Dictionary) -> void:
# 	var send_type: int = Steam.P2P_SEND_RELIABLE
# 	var channel: int = 1
# 	var this_data: PackedByteArray
# 	this_data.append_array(var_to_bytes(payload))
# 	if lobby_members.size() > 1:
# 		for member in lobby_members:
# 			var target_id: int = member["steam_id"]
# 			if target_id != steam_id and target_id != original_sender_id:
# 				Steam.sendP2PPacket(target_id, this_data, send_type, channel)
